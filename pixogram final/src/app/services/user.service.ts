...
export class UserService {
  private baseUrl = 'http://localhost:2222/user';

  private followUrl='http://localhost:4444/follow'
  ...
  getFollowing(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:4444/follow/following/`+`${userid}`)
  }

  getFollower(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:4444/follow/follower/`+`${userid}`)
  }
